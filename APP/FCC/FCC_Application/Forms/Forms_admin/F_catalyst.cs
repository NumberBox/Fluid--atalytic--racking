...
namespace FCC_Application.Forms.Forms_admin {
    public partial class F_catalyst : Form {
        ...
        public F_catalyst(bool add, AdminForm form1, List<string> list) {
            InitializeComponent();
            add_f = add;
            form = form1;
            this.list = list;
        ...
        private void button2_Click(object sender, EventArgs e) {//добавить сохранить
            try {
                if (add_f) {//добавить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "INSERT INTO [CATALYST] ([id], [name]) " +
                                         "VALUES(@id, @name)";
                ...
                if (!add_f) {//сохранить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "DELETE FROM [CATALYST] WHERE [id] = @id AND [name] = @name";
                    SQLiteCommand command = new SQLiteCommand(commandText, connection);
                    command.Parameters.AddWithValue("@id", list[0]);
                    command.Parameters.AddWithValue("@name", list[1]);
                    command.ExecuteNonQuery();
                    commandText = "INSERT INTO [CATALYST] ([id], [name]) " +
                                         "VALUES(@id, @name)";
        ...
        private void button3_Click(object sender, EventArgs e) {
            ...
            commandText = "DELETE FROM [TECH_REG] WHERE [id_catalyst] = @id";
            command = new SQLiteCommand(commandText, connection);
            command.Parameters.AddWithValue("@id", list[0]);
            command.ExecuteNonQuery();

            commandText = "DELETE FROM [NUM_PARAM_CATALYST] WHERE [id_catalyst] = @id";
            command = new SQLiteCommand(commandText, connection);
            command.Parameters.AddWithValue("@id", list[0]);
            command.ExecuteNonQuery();

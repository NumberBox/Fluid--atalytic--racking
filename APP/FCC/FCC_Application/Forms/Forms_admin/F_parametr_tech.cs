...
namespace FCC_Application.Forms.Forms_admin {
    public partial class F_parametr_tech : Form {

        private bool add_f;
        public static AdminForm form;
        List<string> list = new List<string>();
        public F_parametr_tech(bool add, AdminForm form1) {
            InitializeComponent();
            add_f = add;
            form = form1;
            SQLiteConnection connection = new SQLiteConnection("Data Source=data//object_catalysts_tech.db3;Version=3;");
            connection.Open();
            string sqlQuery = "SELECT PARAMETR_TECH_REG.id FROM PARAMETR_TECH_REG";
        ...
        public F_parametr_tech(bool add, AdminForm form1, List<string> list) {
            InitializeComponent();
            add_f = add;
            form = form1;
            this.list = list;
        ...
        private void button2_Click(object sender, EventArgs e) {
            try {
                if (add_f) {//добавить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data//object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "INSERT INTO [PARAMETR_TECH_REG] ([id], [name], [dimension]) " +
                ...
                if (!add_f) {//сохранить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data//object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "DELETE FROM [PARAMETR_TECH_REG] WHERE [id] = @id";
                    SQLiteCommand command = new SQLiteCommand(commandText, connection);
                    command.Parameters.AddWithValue("@id", list[0]);
                    command.ExecuteNonQuery();
                    commandText = "INSERT INTO [PARAMETR_TECH_REG] ([id], [name], [dimension]) " +
                                         "VALUES(@id, @name, @demension)";
        ...
        private void button3_Click(object sender, EventArgs e) {
            ...
            commandText = "DELETE FROM [TECH_REG] WHERE [id_parametr] = @id";

...
namespace FCC_Application.Forms.Forms_admin {
    public partial class F_numbers : Form {

        private bool add_f;
        public static AdminForm form;
        List<string> list = new List<string>();
        bool check_raw_catalyst = false;

        public F_numbers(bool add, AdminForm form1) {
            InitializeComponent();
            setcombobox();
            ...
            connection.Open();
            string sqlQuery = "SELECT NUM_PARAM_EQUIPMENT.id FROM NUM_PARAM_EQUIPMENT";
        ...
        public F_numbers(bool add, AdminForm form1, List<string> list) {
            InitializeComponent();
            setcombobox();
            add_f = add;
            form = form1;
            ...
            if (comboBox2.Text=="Тип применяемого катализатора") {
                ...
                if (Table_Departure.Rows.Count > 0) {
                    ...
                    comboBox3.Text = list[3];
                }
                textBox2.Visible = false;
                comboBox3.Visible = true;
            }
            if (comboBox2.Text == "Тип применяемого сырья") {
                comboBox3.Items.Clear();
        ...
        private void setcombobox() {
            SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
            connection.Open();
            string sqlQuery = "SELECT STAMP.name FROM STAMP";
            SQLiteDataAdapter adapter_Departure = new SQLiteDataAdapter(sqlQuery, connection);
            DataTable Table_Departure = new DataTable();
            adapter_Departure.Fill(Table_Departure);
            if (Table_Departure.Rows.Count > 0) {
            ...
            sqlQuery = "SELECT PARAMETR_EQUIPMENT.name FROM PARAMETR_EQUIPMENT";
        ...
        private string setcombobox2_parametr(int id) {
            SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
            connection.Open();
            string sqlQuery = "SELECT PARAMETR_EQUIPMENT.name FROM PARAMETR_EQUIPMENT WHERE [id] = " + id.ToString() + "";
        ...
        private string setcombobox2_parametr(string name) {
            ...
            SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
            connection.Open();
            string sqlQuery = "SELECT PARAMETR_EQUIPMENT.id FROM PARAMETR_EQUIPMENT WHERE [name] = \"" + name.ToString() + "\"";
            SQLiteDataAdapter adapter_Departure = new SQLiteDataAdapter(sqlQuery, connection);
            DataTable Table_Departure = new DataTable();
            adapter_Departure.Fill(Table_Departure);
            connection.Close();
        ...
        private void button2_Click(object sender, EventArgs e) {
            try {
                if (add_f) {//добавить
                    ...
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "INSERT INTO [NUM_PARAM_EQUIPMENT] ([id], [id_stamp], [id_parametr], [number]) " +
                                         "VALUES(@id, @id_stamp, @id_parametr, @number)";
                ...
                if (!add_f) {//сохранить
                    ...
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data\\object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "DELETE FROM [NUM_PARAM_EQUIPMENT] WHERE [id] = @id";
                    SQLiteCommand command = new SQLiteCommand(commandText, connection);
                    command.Parameters.AddWithValue("@id", list[0]);
                    command.ExecuteNonQuery();
                    commandText = "INSERT INTO [NUM_PARAM_EQUIPMENT] ([id], [id_stamp], [id_parametr], [number]) " +
                                          "VALUES(@id, @id_stamp, @id_parametr, @number)";
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e) {
            if (comboBox2.Text == "Тип применяемого катализатора") {
                ...
                textBox2.Visible = false;
                comboBox3.Visible = true;
                check_raw_catalyst = true;
            }
            else if (comboBox2.Text == "Тип применяемого сырья") {
                comboBox3.Items.Clear();
                SQLiteConnection connection = new SQLiteConnection("Data Source=data\\raw.db3;Version=3;");
                connection.Open();
                string sqlQuery = "SELECT RAW.name FROM RAW";
                ...
                if (check_raw_catalyst) {
                    textBox2.Text = "";
                }
                check_raw_catalyst = false;

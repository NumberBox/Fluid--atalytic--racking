...
namespace FCC_Application.Forms.Forms_admin {
    public partial class F_raw : Form {
        ...
        public F_raw(bool add, AdminForm form1, List<string> list) {
            InitializeComponent();
            add_f = add;
            form = form1;
            this.list = list;
        ...
        private void button2_Click(object sender, EventArgs e) {//добавить сохранить
            try {
                if (add_f) {//добавить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data\\raw.db3;Version=3;");
                    connection.Open();
                    string commandText = "INSERT INTO [RAW] ([id], [name]) " +
                                         "VALUES(@id, @name)";
                ...
                if (!add_f) {//сохранить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data\\raw.db3;Version=3;");
                    connection.Open();
                    string commandText = "DELETE FROM [RAW] WHERE [id] = @id AND [name] = @name";
                    SQLiteCommand command = new SQLiteCommand(commandText, connection);
                    command.Parameters.AddWithValue("@id", list[0]);
                    command.Parameters.AddWithValue("@name", list[1]);
                    command.ExecuteNonQuery();
                    commandText = "INSERT INTO [RAW] ([id], [name]) " +
                                         "VALUES(@id, @name)";
        ...
        private void button3_Click(object sender, EventArgs e) {
            ...
            commandText = "DELETE FROM [NUMERICAL_CHARACTERISTIC_RAW] WHERE [id_raw] = @id";

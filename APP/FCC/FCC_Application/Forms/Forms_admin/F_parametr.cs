...
namespace FCC_Application.Forms.Forms_admin {
    public partial class F_parametr : Form {

        private bool add_f;
        public static AdminForm form;
        List<string> list = new List<string>();
        public F_parametr(bool add, AdminForm form1) {
            InitializeComponent();
            add_f = add;
            form = form1;
            SQLiteConnection connection = new SQLiteConnection("Data Source=data//object_catalysts_tech.db3;Version=3;");
            connection.Open();
            string sqlQuery = "SELECT PARAMETR_EQUIPMENT.id FROM PARAMETR_EQUIPMENT";
        ...
        public F_parametr(bool add, AdminForm form1, List<string> list) {
            InitializeComponent();
            add_f = add;
            form = form1;
            this.list = list;
        ...
        private void button3_Click(object sender, EventArgs e) {
            ...
            SQLiteConnection connection = new SQLiteConnection("Data Source=data//object_catalysts_tech.db3;Version=3;");
            connection.Open();
            string commandText = "DELETE FROM [PARAMETR_EQUIPMENT] WHERE [id] = @id";
            SQLiteCommand command = new SQLiteCommand(commandText, connection);
            command.Parameters.AddWithValue("@id", list[0]);
            command.ExecuteNonQuery();

            commandText = "DELETE FROM [NUM_PARAM_EQUIPMENT] WHERE [id_parametr] = @id";
        ...
        private void button2_Click(object sender, EventArgs e) {
            try {
                if (add_f) {//добавить
                    SQLiteConnection connection = new SQLiteConnection("Data Source=data//object_catalysts_tech.db3;Version=3;");
                    connection.Open();
                    string commandText = "INSERT INTO [PARAMETR_EQUIPMENT] ([id], [name], [dimension]) " +
                ...
                if (!add_f) {//сохранить
                    ...
                    command.Parameters.AddWithValue("@id", list[0]);
                    command.ExecuteNonQuery();
                    commandText = "INSERT INTO [PARAMETR_EQUIPMENT] ([id], [name], [dimension]) " +
                                         "VALUES(@id, @name, @demension)";
